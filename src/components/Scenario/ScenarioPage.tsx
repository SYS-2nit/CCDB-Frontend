...
const ScenarioPage: React.FC = () => {
  ...
  const [selected, setSelected] = React.useState<Partial<Record<ScenarioId, boolean>>>({});
  const [duration, setDuration] = React.useState<number>(90);
  const [running, setRunning] = React.useState(false);
  const [busy, setBusy] = React.useState(false);
  const [status, setStatus] = React.useState<{ current?: ScenarioId; remain?: number; loop?: number }>({});
  ...
  React.useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        setStatus({ current: st.currentScenarioId, remain: st.remainingSec, loop: st.loopCount });
      } catch (e) {
  ...
  const toggle = (id: ScenarioId) => setSelected((prev) => ({ ...prev, [id]: !prev[id] }));
